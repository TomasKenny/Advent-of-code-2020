...
int main(int argc, const char * argv[]) {
    ...
    //------------------------------------------------
    std::function<int(const std::string & bag)> GetBagSize;
    GetBagSize = [&](const std::string & bag) -> int {
        int size = 1;
        for(const auto & childBag: allBags[bag]){
            size += childBag.first * GetBagSize(childBag.second);
        }
        return size;
    };
    printf("shiny gold bag size: %d\n", GetBagSize("shiny gold") - 1);
